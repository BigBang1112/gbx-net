...
public partial class GameBox
{
    ...
    /// <summary>
    /// Parses only the header of the GBX and returns the header parts of the node.
    /// </summary>
    /// <typeparam name="TNode">Known node of the GBX file parsed. Unmatching node will throw an <see cref="InvalidCastException"/>. Nodes to use are located in the GBX.NET.Engines namespace.</typeparam>
    /// <typeparam name="THeaderInterface">A header implementation of <typeparamref name="TNode"/> to make it easier what is in header and what isn't. Usable implementation is typically a nested IHeader interface in the specified <typeparamref name="TNode"/>.</typeparam>
    /// <param name="stream">Stream to read GBX format from.</param>
    /// <param name="progress">Callback that reports any read progress.</param>
    /// <param name="readRawBody">If the body should be read in raw bytes. True allows modification (write abilities) of GBX headers.</param>
    /// <param name="logger">Logger.</param>
    /// <returns>A <see cref="NET.Node"/> casted to <typeparamref name="TNode"/>.</returns>
    /// <exception cref="InvalidCastException"/>
    /// <exception cref="GameBoxParseException"></exception>
    /// <exception cref="TextFormatNotSupportedException">Text-formatted GBX files are not supported.</exception>
    public static THeaderInterface ParseNodeHeader<TNode, THeaderInterface>(Stream stream,
        IProgress<GameBoxReadProgress>? progress = null,
        bool readRawBody = false,
        ILogger? logger = null)
        where TNode : Node, INodeHeader<TNode>
        where THeaderInterface : class, INodeHeader<TNode>
    {
        return (THeaderInterface)(INodeHeader<TNode>)ParseHeader<TNode>(stream, progress, readRawBody, logger).Node;
    }
    ...
    /// <exception cref="GameBoxParseException"></exception>
    /// <exception cref="TextFormatNotSupportedException">Text-formatted GBX files are not supported.</exception>
    public static THeaderInterface ParseNodeHeader<TNode, THeaderInterface>(string fileName,
        IProgress<GameBoxReadProgress>? progress = null,
        bool readRawBody = false,
        ILogger? logger = null)
        where TNode : Node, INodeHeader<TNode>
        where THeaderInterface : class, INodeHeader<TNode>
    {
        return (THeaderInterface)(INodeHeader<TNode>)ParseHeader<TNode>(fileName, progress, readRawBody, logger).Node;
    }
}

...
public partial class GameBox
{
    public class Body
    {
                                           ...
                                           ILogger? logger)
        {
            ...
            ReadMainNode(node, data, uncompressedSize, reader.Settings.StateGuid, progress, logger);
        }
                                             ...
                                             ILogger? logger)
        {
            ...
            ReadMainNode(node, uncompressedData, reader.Settings.StateGuid, progress, logger);
        }
                                         ...
                                         ILogger? logger)
        {
            ...
            ReadMainNode(node, buffer, contextGuid, progress, logger);
        }
                                         ...
                                         ILogger? logger)
        {
            ...
            ReadMainNode(node, ms, contextGuid, progress, logger);
        }

        /// <exception cref="NodeNotImplementedException">Auxiliary node is not implemented and is not parseable.</exception>
        /// <exception cref="ChunkReadNotImplementedException">Chunk does not support reading.</exception>
        /// <exception cref="IgnoredUnskippableChunkException">Chunk is known but its content is unknown to read.</exception>
        private static void ReadMainNode(Node node, Stream stream, Guid? contextGuid, IProgress<GameBoxReadProgress>? progress, ILogger? logger)
        {
                                                    ...
                                                    CancellationToken cancellationToken)
        {
            ...
            await ReadMainNodeAsync(node, buffer, stateGuid, logger, asyncAction, cancellationToken);
        }
        ...
        {
            using var ms = new MemoryStream(data);
            await ReadMainNodeAsync(node, ms, stateGuid, logger, asyncAction, cancellationToken);
        }
